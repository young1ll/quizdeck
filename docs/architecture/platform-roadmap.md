...
| **L2** | Cloudflare Tunnel | 경계/IGW · 인바운드 0 · 홈 IP 은닉 | 다음 |
...
- [ ] **Cloudflare Tunnel 구성 (L2, 다음)** — `cloudflared`(k3s Deployment) + 터널 토큰(Secret) + 공개 호스트명 → Traefik/quizdeck 라우팅. ⚠️ 사용자 Cloudflare 계정 + Cloudflare 관리 도메인 필요. 현재는 타넷 내부(`http://100.81.230.113/`)에서만 접근.
- [ ] **postgres 외부 구성 (보류)** — quizdeck가 현재 정적(localStorage)이라 불필요. 서버측 진도 동기화·인증 도입 시 착수. ⚠️ VM→호스트 OVS 격리로 "호스트 postgres"는 불가 → **별도 DB VM(게스트↔게스트 가능)** 또는 VM내 컨테이너로.
- [ ] L4~L6(관측성→mesh→Meshery)은 외부 노출까지 끝난 뒤 착수.
