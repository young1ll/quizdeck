...
- [ ] **VMM 설치 + Linux VM 생성** — DSM Package Center에서 Virtual Machine Manager 설치(HW VT-x 있음) → 경량 Linux VM(예: Debian/Ubuntu, 2 vCPU·4GB·20GB) 생성·OS 설치 → VM에 SSH 가능하게.
- [ ] **VM 안에 k3s 설치** — `curl -sfL https://get.k3s.io | sh -` (modern 커널이라 정상 기동). Traefik 기본 포함.
