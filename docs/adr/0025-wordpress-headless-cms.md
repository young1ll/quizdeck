...
## 애던덤 — 미디어 R2 offload 완결 + 인프라 연계 감사 (2026-07-14)

연계 감사(그릴링 세션)로 결정 7 의 미완 항목("미디어는 R2 offload 2단계")이 **구조적 결함**
이었음을 확인: emptyDir 라 업로드가 pod 재시작에 소실될 뿐 아니라, `get_the_post_thumbnail_url`
이 만드는 URL 의 호스트(wp.myquizdeck.com)가 **tailnet 전용이라 공개 사용자에겐 애초에 안
보인다**. 라이브 실측으로 미디어 0건·image 필드 0건 확인(이관 데이터에 이미지 없음) — 복구
없이 전방향 구현만.

- **quizdeck-media mu-plugin**(자체구현 — ACF 결정과 같은 결): 중간 사이즈·-scaled 비활성
  (서빙 계약이 'full' 뿐) + 업로드 시 R2 PUT(SigV4 자체구현, SDK 미도입) + `_qd_r2_key` 메타
  → `wp_get_attachment_url` 필터로 `media.myquizdeck.com`(R2 커스텀 도메인 — r2.dev 는
  프로덕션 부적격) URL 파생. 첨부 메타가 MariaDB 에 있으므로 pod 재시작과 무관. env 5종
  (`QD_MEDIA_*`, wp-media Secret — optional) 미완이면 전체 no-op.
  **SigV4 함정**: 서명의 host 는 전송 Host 헤더와 바이트 일치 — 비표준 포트 포함(로컬 MinIO
  :9000 에서 SignatureDoesNotMatch 로 실측).
- **자격증명 재사용**: 구 quizdeck-auth 의 `R2_MEDIA_*` 가 media 버킷 스코프임을 실증
  (ListBuckets Access Denied·media 버킷 ls OK) → wp-media Secret 으로 이전, quizdeck-auth
  에서 `PAYLOAD_SECRET` 과 함께 제거(ADR-0025 4단계 잔여물 정산).
- **동반 정리**: `@payload-config` 죽은 별칭·Payload stale 주석 3곳·빈 content/ 트리 제거.
  cloudflared 이미지 `latest`→라이브 실측 버전(2026.6.1) 고정 — 공개 진입로의 GitOps 원칙
  일치.
- **감사에서 정상 실증된 연계**(변경 없음): 웹훅 토큰 양측 일치 + 실호출 200, 백업 CronJob
  2종 성공, 빌드 시점 WP 무의존. **보류**: Google OAuth(org_internal — 콘솔 작업)·Naver
  검수 상태 확인.
- 검증: 로컬 WP+MariaDB+MinIO 풀 스택 — 업로드→객체 생성(Content-Type·immutable 캐시 헤더)
  →`get_the_post_thumbnail_url` 이 미디어 도메인 반환→첨부 삭제 시 객체 삭제. 테스트 322·
  tsc 0·kustomize 3 디렉토리 빌드 통과.
