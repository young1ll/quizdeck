...
```sh
# stringData — 평문 그대로(서버가 base64 변환). data+base64 는 리눅스 base64 의 76자
# 줄바꿈이 JSON 을 깨는 함정(2026-07-10 실사)이 있어 쓰지 않는다.
kubectl -n quizdeck patch secret quizdeck-auth --type merge -p "{\"stringData\":{
  \"PAYLOAD_SECRET\":\"$(openssl rand -base64 32 | tr -d '\n')\",
  \"R2_MEDIA_ENDPOINT\":\"https://<account-id>.r2.cloudflarestorage.com\",
  \"R2_MEDIA_BUCKET\":\"quizdeck-media\",
  \"R2_MEDIA_ACCESS_KEY_ID\":\"<access-key-id>\",
  \"R2_MEDIA_SECRET_ACCESS_KEY\":\"<secret-access-key>\"}}"
kubectl -n quizdeck rollout restart deploy/quizdeck
```
