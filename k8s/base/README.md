...
better-auth 의 세션·JWT 서명 키(`BETTER_AUTH_SECRET`)와 Better Auth Infra
대시보드 API 키(`BETTER_AUTH_API_KEY`). kubectl 호스트에서 1회:

```sh
kubectl -n quizdeck create secret generic quizdeck-auth \
  --from-literal=BETTER_AUTH_SECRET="$(openssl rand -base64 32)" \
  --from-literal=BETTER_AUTH_API_KEY="ba_..."   # dash.better-auth.com 발급
```
