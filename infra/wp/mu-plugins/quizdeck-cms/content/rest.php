...
function qd_rest_ingest(array $value, WP_Post $post)
{
    ...
    // 순서 자동 부여(다이어그램·개념 카드) — 스키마 제외(fields.php)로 REST 봉투에 ord 가 없다.
    // exam 확정 직후 여기서.
    if (in_array($type, ['qd_diagram', 'qd_concept'], true) && (string) get_post_meta($post->ID, 'qd_ord', true) === '') {
        $eid = (int) get_post_meta($post->ID, 'qd_exam_id', true);
        if ($eid) update_post_meta($post->ID, 'qd_ord', (string) qd_next_ord($type, $eid));
    }
    if ($type === 'qd_question') {
