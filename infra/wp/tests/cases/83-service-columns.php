...
t_assert(isset($cols['qd_col_icon'], $cols['qd_provider'], $cols['qd_cat']), '컬럼 3종 등록 (아이콘·provider·분류)');
t_assert(array_keys($cols)[2] === 'qd_col_icon', '컬럼 위치 — 제목 다음');
...
t_assert(str_contains($cell('qd_col_icon'), '<span') || str_contains($cell('qd_col_icon'), '<img'), '아이콘 셀(이모지 span — 픽스처 🗂️/🧪)');
...
t_assert(str_contains($cell('qd_col_icon'), '<img') && str_contains($cell('qd_col_icon'), 'media.test.invalid'), '대표이미지 → img 셀');
