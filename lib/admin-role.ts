// admin 역할 술어 — 순수(클라-안전) 절반 (ADR-0012 결정 10). lib/admin.ts(서버: getAdminSession, auth=pg
// 의존)와 대칭이되, 이 술어는 클라(exam 맥락 헤더의 '이 시험 편집'·/me 의 '어드민' 조건부 렌더)에서도
// 쓰여 서버 전용 lib/auth 를 끌면 클라 번들이 깨진다. learner.ts / learner-server.ts 분리와 같은 결.
// 정규 규칙 = user.role 이 'admin' 포함(better-auth admin 플러그인 adminRoles 기본값 ["admin"]).
